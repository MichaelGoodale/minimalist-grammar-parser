...
#[derive(Debug)]
pub struct NeuralGeneratorForOutput<'a, B: Backend> {
    lexicon: &'a NeuralLexicon<B>,
    parses: ParseHeap<'a, usize, NeuralBeam<'a, B>>,
    move_log_prob: NeuralProbability,
    merge_log_prob: NeuralProbability,
}

impl<'a, B: Backend> NeuralGeneratorForOutput<'a, B> {
    pub fn new(
        lexicon: &'a NeuralLexicon<B>,
        g: &'a GrammarParameterization<B>,
        targets: Option<&'a [Vec<usize>]>,
        max_string_length: usize,
        config: &'a NeuralConfig,
    ) -> NeuralGeneratorForOutput<'a, B> {
        let mut parses =
            MinMaxHeap::with_capacity(config.parsing_config.max_beams.unwrap_or(100000));
        parses.extend(NeuralBeam::new(lexicon, g, 0, targets, max_string_length).unwrap());
        NeuralGeneratorForOutput {
            lexicon,
            move_log_prob: NeuralProbability(
                NeuralProbabilityRecord::MoveRuleProb,
                None,
            ...
            parses: ParseHeap::new(parses, &config.parsing_config),
        }
    }
}

impl<'a, B: Backend> Iterator for NeuralGeneratorForOutput<'a, B> {
    type Item = CompletedParse;
