...
pub fn get_neural_outputs<B: Backend>(
    ...
    dbg!(loss_per_grammar.clone().detach());
    //let p_of_t_given_p: Tensor<B, 1> = log_sum_exp_dim(loss_per_grammar, 0).squeeze(0);
    ...
    (
        log_sum_exp_dim(loss_per_grammar.sum_dim(0).squeeze(0), 0),
        n_compatible,
    )
}
